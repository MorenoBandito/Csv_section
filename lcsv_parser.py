...
#    input_csv = raw_input('Enter Filename (ex: yourfile.csv)   ')
...
#countdown for file completion [May change for python 2]
def countdown(t):
...
def Csv_parser():
    ...
    if len(sys.argv) == 2 and existence() == True:
        #output_csv = str(datetime.datetime.now().strftime("%Y%m%d_%H_%M_%p"))+' '+sys.argv[1][9:26] #name of new file
...
def ts():
    ...
    for line in open(sys.argv[1]or input_csv):
        line = line.strip()
        Timestamp = re.search(r'^\d+:\d{2}:\d{2}.\d+', line)
        if Timestamp:
            T= 'T'
...
if __name__=="__main__":
	...
	if len(sys.argv) != 2:
    		print("##ERROR##  Invalid command line paramerters !!!\n")
		print("##INFO##   Usage:")
		print("           python ./Csv_parser.py  <yourfile.csv>\n")
	else:
    		pass
